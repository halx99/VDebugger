...
BOOL CALLBACK EnumerateLoadedModulesProc64(PCTSTR  ModuleName, DWORD64 ModuleBase, ULONG   ModuleSize, PVOID   UserContext);
...
void VDbg::StartDebugging(LPCWSTR path)
{
    if (!CreateProcess(
        ...
        NULL,
        FALSE,
        DEBUG_ONLY_THIS_PROCESS || CREATE_SUSPENDED | CREATE_NEW_CONSOLE,
        ...
        &pi)) 
    {
        mythread->setGeneralOutput("create process failed \n");
        return;
    }
    bool wait = true;
...
void VDbg::OnProcessCreated()
{
	...
	{
        mythread->setGeneralOutput("done.");
	}
	else
	{
        mythread->setGeneralOutput("symbols initialize failed");
	}
    mythread->NotifyInitDone();
...
void VDbg::OnException()
{
	...
	switch(debug_event.u.Exception.ExceptionRecord.ExceptionCode)
	{
     ...
     case EXCEPTION_BREAKPOINT:
			stop_addr = peekEIP() - 1;
            if(BPs.find(stop_addr) == BPs.end()) break; // ignore all non-user break points
			setTrap();
			setEIP(-1);
...
void VDbg::OnOutputDebugString()
{
   ...
   string temp(msg);
   mythread->setGeneralOutput(temp);
   delete []msg;
}

bool VDbg::Symbol_Initialize()
{
    mythread->setGeneralOutput("Reading symbols ...");
	if (SymInitialize(getDebuggeeProcessHandle(), NULL, FALSE) == TRUE) {
		...
		if (moduleAddress == 0) {
			if(GetLastError() != 0)
			{
                mythread->setGeneralOutput("unable to Load Module, err code: " + GetLastError());
				ShowVdbg();
				return false;
			}
			else
		...
		{
            mythread->setGeneralOutput("unable to  Get Module Info, err code: " + GetLastError());
			ShowVdbg();
			return false;
        }
        SymEnumSourceFiles(getDebuggeeProcessHandle(), moduleAddress, "*.cpp", EnumSourceFilesProc, this);
        SymEnumSourceFiles(getDebuggeeProcessHandle(), moduleAddress, "*.h", EnumSourceFilesProc, this);
        SymEnumSourceFiles(getDebuggeeProcessHandle(), moduleAddress, "*.hpp", EnumSourceFilesProc, this);
 ...
 * @param addr 要取消断点的地址
 */
void VDbg::cancelBreakPoint(DWORD addr)
{
	if(BPs.find(addr) == BPs.end())
	{
        mythread->setGeneralOutput("no break point on this line .");
		return;
	}
...
string VDbg::GetTypeValue(int typeID, DWORD modBase, BYTE* pData)
{
    ...
    switch (typeTag) {
        ...
        case SymTagPointerType:
            return GetPointerTypeValue(pData);
 ...
 * @param pData 变量存放的地址
 */
string VDbg::GetPointerTypeValue(BYTE* pData)
{
	ostringstream valueBuilder;
...
void VDbg::Print_Var(string var)
{
	...
	if (symbol_map.find(var) != symbol_map.end())
	{
        ...
        str  = GetTypeValue(symbol_map[var].type, (DWORD)symbol_map[var].modBase, (BYTE*)var_addr);
	}
	else //
	{
		...
		if(local_symbol_map.find(var) != local_symbol_map.end())
			...
			return;
        str  = GetTypeValue(local_symbol_map[var].type, (DWORD)local_symbol_map[var].modBase, (BYTE *)var_addr);
	}
	
    mythread->setGeneralOutput(str);
}
/**
 * 打印内存某内存地址处的数据
 * @param Address 地址
...
void VDbg::Print_LocalVars()
{
	...
	for(auto it = local_symbol_map.begin(); it != local_symbol_map.end(); it++)
	{
        ...
        string str  = GetTypeValue(it->second.type, (DWORD)it->second.modBase, (BYTE *)var_addr);
		string temp(it->second.name);
        mythread->setGeneralOutput(it->second.name + ": "+ "\n" + str + "\n");
	}
}
/**
 * 打印所有全局变量
 ...
 * @param bInvisible true表示为一次性断点，否则不是。
 */
void VDbg::CancelBreakPointBySourceLine(char * szFileName, int lineNumber)
{
    cancelBreakPoint(getAddrBySourceLine(szFileName, lineNumber));
}
/**
 * 根据文件名和行号取消断点
...
{
    unsigned int size = enum_files.size();
...
bool VDbg::ShowSourceLine(DWORD addr, bool bRaw)
{
	...
	if(lineInfo == NULL)
	{
        ...
        mythread->setGeneralOutput("fail to read lineInfo from address err code: " + GetLastError() + '\n');
		return false;
	}
    ...
    string output = strTrim((source_file_map[lineInfo->FileName])[lineInfo->LineNumber - 1]);
    if(!bRaw)
        output = "=>" + output;
    mythread->setGeneralOutput(output+"\n");
...
DWORD VDbg::getAddrBySourceLine(char * szFileName, int lineNumber)
{
	...
	long displacement;
    SymGetLineFromName64(getDebuggeeProcessHandle(), "VDbgSymbol", szFileName, lineNumber,
										 &displacement,
										 &imghlp_line64);
    return (DWORD)imghlp_line64.Address;
}
/**
 * 根据地址得到对应的行的信息
 * @param addr 地址
...
{
    return (DWORD)getCurrStackFrame()->AddrReturn.Offset;
}
/**
 * 打印命令输入处的提示
 */
...
BOOL CALLBACK SymEnumSymbolsLocalProc(PSYMBOL_INFO pSymInfo, ULONG SymbolSize, PVOID UserContext)
{
    SymbolSize = 0; // just to kill the warning
	if(pSymInfo->Tag == SymTagData)
	{
        ...
        my_sym_info->Address = (DWORD)pSymInfo->Address;
...
BOOL __stdcall EnumSourceFilesProc(PSOURCEFILE pSourceFile, PVOID UserContext)
{
    VDbg * eumDbg = (VDbg*) UserContext;
    string temp(pSourceFile->FileName);
    eumDbg->enum_files.push_back(temp);
    return true;
}
